    ...
    >
      <div className="flex items-center justify-center h-20 border-b border-[var(--color-border)] shrink-0 px-4">
        <img
          ...
          className={`transition-all duration-300 ${isSidebarCollapsed ? 'h-14 w-14' : 'h-12 w-12'}`}
        />
        <AnimatePresence>
          {!isSidebarCollapsed && (
            <motion.h1
              ...
              className="ml-4 text-2xl font-bold text-[var(--color-text-primary)] leading-tight"
            >
              Portal Docente
            </motion.h1>
          )}
        ...
        <ul className="space-y-1">
          {teacherNavItems.map((item) => (
            <li key={item.to} onMouseEnter={() => setHoveredItem(item.to)} onMouseLeave={() => setHoveredItem(null)}>
              ...
              >
                <item.icon
                  className={`h-8 w-8 shrink-0 transition-all ${isSidebarCollapsed ? 'mx-auto' : 'mr-5'}`}
                />
                <AnimatePresence>
                  {!isSidebarCollapsed && (
                    <motion.span
